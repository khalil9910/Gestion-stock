...
CMD ["/bin/sh", "-lc", "set -e; rm -f /etc/apache2/mods-enabled/mpm_*.load /etc/apache2/mods-enabled/mpm_*.conf; a2enmod mpm_prefork rewrite >/dev/null 2>&1 || true; : \"${PORT:=80}\"; sed -ri -e \"s/^Listen[[:space:]]+80$/Listen ${PORT}/g\" /etc/apache2/ports.conf; sed -ri -e \"s/<VirtualHost[[:space:]]+\\\\*:80>/<VirtualHost *:${PORT}>/g\" /etc/apache2/sites-available/000-default.conf; exec apache2-foreground"]
