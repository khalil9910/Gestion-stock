<x-app-layout>
    <x-slot name="header">
        <div>
            <h5 class="mb-1">{{ __('Archive entrees') }}</h5>
            ...
            <div class="row gx-3 align-items-center">
                ...
                <div class="col-auto">
                    <div class="d-flex gap-2 flex-wrap">
                        <a class="btn btn-link btn-sm" href="{{ route('stock.entrees.index') }}">{{ __('Retour liste') }}</a>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <tbody>
                        @foreach ($entrees as $entree)
                            <tr>
                                ...
                                <td>{{ $entree->deleted_at?->format('Y-m-d H:i') }}</td>
                                <td class="text-end">
                                    <form class="d-inline" method="POST" action="{{ route('stock.entrees.restore', $entree->id) }}" onsubmit="return confirm('Restaurer cette entree ?');">
                                        @csrf
